...
const getCurrentUser=asyncHandler(async(req,res)=>{
    return res
    .status(200)
    .json(new ResponseApi(200,req.user,"current user fetched succesfully"))
})
...
const getUserChannelProfile = asyncHandler(async(req,res)=>{
    const {username} = req.params //get username from req.params means url se

    if(!username?.trim()){
        throw new ErrorApi(400,"username is missing")
    }

    //aggregrate piplelines returns value in array
    ...
    if(!channel?.length){
        throw new ErrorApi(404,"Channel doesnot exist")
    }

    return res
    .status(200)
    .json(
        new ResponseApi(200,channel[0],"user channel fetched successfully")
    )
